...
const handleLogin = async () => {
  ...
  if (state.username === 'admin' && state.password === 'password') {
    toast.add({
      ...
      description: 'Welcome back to your BiblioCore dashboard.',
      color: 'success',
      icon: 'i-lucide-check-circle'
    })
    navigateTo('/products/elib/dashboard')
  } else {
    toast.add({
      title: 'Login Failed',
...
<template>
  <div class="theme-elib min-h-screen bg-muted/30 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      <div class="flex justify-center">
        <!-- BiblioCore Logo -->
        <NuxtLink to="/products/elib" class="flex items-center gap-2.5 transition-transform hover:scale-105">
          <div class="size-12 rounded-xl bg-primary flex items-center justify-center shadow-sm">
            <UIcon name="i-lucide-library" class="size-7 text-white" />
          </div>
          <span class="font-bold text-3xl tracking-tight text-highlighted">
            Biblio<span class="text-primary">Core</span>
          </span>
        </NuxtLink>
      </div>
      <p class="mt-4 text-center text-sm text-muted">Your complete Electronic Library Management System</p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px]">
      <UCard class="shadow-sm rounded-2xl" :ui="{ body: 'px-4 py-8 sm:p-10' }">
        <div class="mb-8">
          ...
          <p class="mt-2 text-center text-sm text-muted">
            Sign in to your BiblioCore dashboard
          </p>
        </div>
        <UForm :schema="schema" :state="state" class="space-y-6" @submit="handleLogin">
          ...
          <div class="relative overflow-hidden rounded-lg bg-primary/5 border border-primary/10 px-4 py-3 text-xs text-muted">
            ...
            <div class="absolute right-2 -bottom-3">
              <UIcon name="i-lucide-library" class="size-14 text-primary opacity-10" />
            </div>
          </div>
      ...
      <p class="mt-8 text-center text-sm text-dimmed">
        ...
        <NuxtLink to="/products/elib" class="font-medium text-primary hover:text-primary/80 transition-colors ml-1">
          Contact Sales
        </NuxtLink>
      </p>
    </div>
