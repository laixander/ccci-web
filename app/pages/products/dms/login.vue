...
const handleLogin = async () => {
  ...
  if (state.username === 'admin' && state.password === 'password') {
    toast.add({
      ...
      description: 'Welcome back to your DocuCore dashboard.',
      color: 'success',
      icon: 'i-lucide-check-circle'
    })
    navigateTo('/products/dms/dashboard')
  } else {
    toast.add({
      title: 'Login Failed',
...
<template>
  <div class="theme-dms min-h-screen bg-muted/30 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      <div class="flex justify-center">
        <!-- DocuCore Logo -->
        <NuxtLink to="/products/dms" class="flex items-center gap-2.5 transition-transform hover:scale-105">
          <div class="size-12 rounded-xl bg-primary flex items-center justify-center shadow-sm">
            <UIcon name="i-lucide-folder-open" class="size-7 text-white" />
          </div>
          <span class="font-bold text-3xl tracking-tight text-highlighted">
            Docu<span class="text-primary">Core</span>
          </span>
        </NuxtLink>
      </div>
      <p class="mt-4 text-center text-sm text-muted">Your secure document management platform</p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px]">
      <UCard class="shadow-sm rounded-2xl" :ui="{ body: 'px-4 py-8 sm:p-10' }">
        <div class="mb-8">
          ...
          <p class="mt-2 text-center text-sm text-muted">
            Sign in to your DocuCore dashboard
          </p>
        </div>
      ...
      <p class="mt-8 text-center text-sm text-dimmed">
        ...
        <NuxtLink to="/products/dms" class="font-medium text-primary hover:text-primary/80 transition-colors ml-1">
          Contact Sales
        </NuxtLink>
      </p>
    </div>
