...
const handleLogin = async () => {
  ...
  if (state.username === 'admin' && state.password === 'password') {
    toast.add({
      ...
      description: 'Welcome to your ExecIS dashboard.',
      color: 'success',
      icon: 'i-lucide-check-circle'
    })
    navigateTo('/products/eis/dashboard')
  } else {
    toast.add({
      title: 'Login Failed',
...
<template>
  <div class="theme-eis min-h-screen bg-muted/30 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      <div class="flex justify-center">
        <NuxtLink to="/products/eis" class="flex items-center gap-2.5 transition-transform hover:scale-105">
          <div class="size-12 rounded-xl bg-primary flex items-center justify-center shadow-sm">
            <UIcon name="i-lucide-bar-chart-3" class="size-7 text-white" />
          </div>
          <span class="font-bold text-3xl tracking-tight text-highlighted">
            Exec<span class="text-primary">IS</span>
          </span>
        </NuxtLink>
      </div>
      <p class="mt-4 text-center text-sm text-muted">Executive intelligence for strategic leaders</p>
    </div>

    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px]">
      <UCard class="shadow-sm rounded-2xl" :ui="{ body: 'px-4 py-8 sm:p-10' }">
        <div class="mb-8">
          <h2 class="text-center text-3xl font-extrabold text-highlighted tracking-tight">Welcome back</h2>
          <p class="mt-2 text-center text-sm text-muted">Sign in to your ExecIS dashboard</p>
        </div>
        <UForm :schema="schema" :state="state" class="space-y-6" @submit="handleLogin">
          <UFormField label="Username" name="username">
            <UInput v-model="state.username" type="text" required autocomplete="username" icon="i-lucide-user" placeholder="username" class="w-full" size="lg" />
          </UFormField>

          <UFormField label="Password" name="password">
            <UInput v-model="state.password" type="password" required autocomplete="current-password" icon="i-lucide-lock" placeholder="••••••••" class="w-full" size="lg" />
          </UFormField>

          <div class="flex items-center justify-between mt-4">
            <UCheckbox v-model="rememberMe" label="Remember me" name="remember-me" color="primary" />
            <div class="text-sm">
              <NuxtLink to="#" class="font-medium text-primary hover:text-primary/80 transition-colors">Forgot your password?</NuxtLink>
            </div>
          </div>
          ...
          <div class="pt-2">
            <UButton type="submit" block color="primary" size="xl" :loading="loading" class="font-semibold">Sign in</UButton>
          </div>
        </UForm>

        <div class="mt-8">
          <div class="relative">
            <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-default" /></div>
      ...
      <p class="mt-8 text-center text-sm text-dimmed">
        ...
        <NuxtLink to="/products/eis" class="font-medium text-primary hover:text-primary/80 transition-colors ml-1">Contact Sales</NuxtLink>
      </p>
    </div>
  </div>
</template>
