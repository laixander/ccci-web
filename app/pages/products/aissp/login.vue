...
const handleLogin = async () => {
  ...
  if (state.username === 'admin' && state.password === 'password') {
    toast.add({
      ...
      description: 'Welcome back to your AISSP dashboard.',
      color: 'success',
      icon: 'i-lucide-check-circle'
    })
    navigateTo('/products/aissp/dashboard')
  } else {
    toast.add({
      title: 'Login Failed',
...
<template>
  <div class="theme-aissp min-h-screen bg-muted/30 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      <div class="flex justify-center">
        <!-- AISSP Logo -->
        <NuxtLink to="/products/aissp" class="flex items-center gap-2.5 transition-transform hover:scale-105">
          <div class="size-12 rounded-xl bg-primary flex items-center justify-center shadow-sm">
            <UIcon name="i-lucide-network" class="size-7 text-white" />
          </div>
          <span class="font-bold text-3xl tracking-tight text-highlighted">
            AIS<span class="text-primary">SP</span>
          </span>
        </NuxtLink>
      </div>
      <p class="mt-4 text-center text-sm text-muted">Align ICT investments with strategic goals</p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px]">
      <UCard class="shadow-sm rounded-2xl" :ui="{ body: 'px-4 py-8 sm:p-10' }">
        <div class="mb-8">
          ...
          <p class="mt-2 text-center text-sm text-muted">
            Sign in to your AISSP dashboard
          </p>
        </div>
      ...
      <p class="mt-8 text-center text-sm text-dimmed">
        ...
        <NuxtLink to="/products/aissp" class="font-medium text-primary hover:text-primary/80 transition-colors ml-1">
          Contact Sales
        </NuxtLink>
      </p>
    </div>
