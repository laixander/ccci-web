...
<template>
  <div class="theme-ams">
    ...
    <template #right>
      <UColorModeButton />
      <UButton label="Sign in" color="neutral" variant="ghost" class="hidden sm:flex" to="/products/ams/login" />
    ...
    <template #body>
      ...
      <div class="mt-4 flex flex-col gap-2">
        <UButton label="Sign in" color="neutral" variant="outline" block to="/products/ams/login" />
  ...
  <UFooter>
          ...
          <template #left>
            <div class="space-y-4">
              ...
              <p class="text-muted text-sm leading-relaxed max-w-xs">
                The intelligent asset management system designed to track and optimize your resources.
              </p>
            </div>
          </template>
        </UFooterColumns>
    ...
    <template #left>
      <p class="text-muted text-sm flex items-center gap-2">
        <UIcon name="i-lucide-copyright" /> <span>{{ new Date().getFullYear() }} AssetCore. All rights reserved.</span>
      </p>
    </template>
